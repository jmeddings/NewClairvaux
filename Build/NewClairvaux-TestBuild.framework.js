  ...
  function _StartCompass() {
          ...
          if (typeof DeviceOrientationEvent !== "undefined") {
              ...
              if (typeof DeviceOrientationEvent.requestPermission === "function") {
                  ...
                  // Ask the user for permission to access motion sensors
                  ...
                  // Android and most browsers do not need permission
                  window.addEventListener("deviceorientation", handleOrientation);
              }
  
              // This function runs every time the device orientation changes
              function handleOrientation(event) {
  
                  let heading = 0;
  
                  // iOS provides a direct compass heading
                  if (event.webkitCompassHeading !== undefined) {
                      heading = event.webkitCompassHeading;
  
                  } else {
                      // Android provides alpha (rotation around Z axis)
                      // Convert it so 0 = North
                      heading = 360 - event.alpha;
                  }
  
                  // Send the heading value to Unity
                  // "CompassManager" must match the GameObject name in Unity
                  // "UpdateHeading" must match the C# function name
                  SendMessage("CompassManager", "UpdateHeading", heading);
              }
          }
      }
