...
// end include: preamble.js
  ...
  function _JSLibRequestCamPerms(grantedCallback, deniedCallback, notFoundCallback) {
          navigator.mediaDevices.getUserMedia({ video: { facingMode: { exact: "environment" }, },
          }).then(function (stream) {
              
              // If successful, report camera permissions were granted.
              MyGameInstance.SendMessage('Main Camera', 'SendMsg', "Out: " + stream);
              
              // Sever plugin's connections to devices so that Unity can access them w/o issue.
              ...
              getWasmTableEntry(grantedCallback)();
              
          }).catch(function (err) {
              
              // If a camera wasn't found, call not found callback.
              if (err.name === "NotFoundError") {
                  console.error("Camera Not Found: ", err);
                  getWasmTableEntry(notFoundCallback)();
              }
              
              // Otherwise, call permission denied callback.
              else {
                  console.error("Camera Permissions Denied: ", err);
                  getWasmTableEntry(deniedCallback)();
              }
              
          });
      }

  function _JSLibRequestOrientationPerms() {
  ...
  function _JSLibStartCompass() {
  
          function handleAbsolute(event) {
              if (event.alpha === null) return;
  
              // True heading when absolute is available
              let heading = 360 - event.alpha;
  
              console.log("Absolute heading:", heading);
          ...
          function handleFallback(event) {
              ...
              // Fallback, may be magnetic or relative
              let heading = 360 - event.alpha;
...
var wasmImports = {
  ...
  "JSLibRequestCamPerms": _JSLibRequestCamPerms,
  "JSLibRequestOrientationPerms": _JSLibRequestOrientationPerms,
  "JSLibStartCompass": _JSLibStartCompass,
